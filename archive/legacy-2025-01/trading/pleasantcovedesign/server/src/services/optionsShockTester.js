/**
 * Options Shock Tester for EvoTester (JavaScript version)
 * Pre-trade risk validation with price and volatility shocks
 */

class OptionsShockTester {
  constructor() {
    this.MAX_PREMIUM_RISK = 0.02; // 2% of account max loss per trade
    this.SHOCK_PRICE_SIGMA = 1.0;
    this.SHOCK_IV_PTS = 10;
    this.THETA_BUDGET_MAX = 0.0025; // 0.25% of equity per day
  }

  /**
   * Run comprehensive shock test on proposed options position
   */
  testPosition(position, accountSize, expectedMove, ivRank) {
    const results = {
      passed: true,
      worstCasePnL: 0,
      stressTests: {
   ...
   */
  testPriceShocks(position, expectedMove) {
   ...
   */
  testIVShocks(position, currentIVR) {
   ...
   */
  testCombinedShock(position, expectedMove, ivRank, accountSize) {
   ...
   */
  calculatePnL(position, priceMove) {
   ...
   */
  calculateVerticalPnL(position, priceMove) {
   ...
   */
  calculateLongCallPnL(position, priceMove) {
   ...
   */
  calculateLongPutPnL(position, priceMove) {
    if (!position.longStrike) return 0;
   ...
   */
  calculateCombinedPnL(position, priceMove, newIV, daysDecay) {

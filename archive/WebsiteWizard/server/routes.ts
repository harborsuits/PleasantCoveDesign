...
import { upload, handleFileUpload, serveFile, cleanupOldFiles } from './file-handler';
...
const PUBLIC_API_ROUTES = [
  ...
  "/socket.io", // Socket.IO endpoint needs to be public (auth handled separately)
];
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Serve uploaded files with custom handler (before auth middleware)
  ...
  // Serve client widget test page
  app.get('/client-widget', (req, res) => {
    res.sendFile('client-widget-test.html', { root: process.cwd() });
  });
  ...
  app.get("/api/businesses/stage/:stage", async (req, res) => {
    try {
      ...
      const businesses = await dataStorage.getBusinessesByStage(stage as any);
      res.json(businesses);
    } catch (error) {
  ...
  app.post("/api/businesses", async (req, res) => {
    try {
      ...
      const business = await dataStorage.createBusiness(validatedData);
  ...
  app.put("/api/businesses/:id", async (req, res) => {
    try {
      ...
      const business = await dataStorage.updateBusiness(id, updates);
      res.json(business);
    } catch (error) {
  ...
  app.delete("/api/businesses/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      await dataStorage.deleteBusiness(id);
      res.status(204).send();
    } catch (error) {
  ...
  app.get("/api/campaigns", async (req, res) => {
    try {
      const campaigns = await dataStorage.getCampaigns();
      res.json(campaigns);
    } catch (error) {
  ...
  app.post("/api/campaigns", async (req, res) => {
    try {
      ...
      const campaign = await dataStorage.createCampaign(validatedData);
  ...
  app.put("/api/campaigns/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const updates = req.body;
      const campaign = await dataStorage.updateCampaign(id, updates);
      res.json(campaign);
    } catch (error) {
  ...
  app.post("/api/templates", async (req, res) => {
    try {
      const template = await dataStorage.createTemplate(req.body);
      res.json(template);
    } catch (error) {
  ...
  //   try {
  //     const id = parseInt(req.params.id);
  //     const template = await dataStorage.updateTemplate(id, req.body);
  //     res.json(template);
  //   } catch (error) {
  ...
  //   try {
  //     const id = parseInt(req.params.id);
  //     await dataStorage.deleteTemplate(id);
  //     res.status(204).send();
  //   } catch (error) {
  ...
  app.get("/api/activities", async (req, res) => {
    try {
      ...
      const activities = await dataStorage.getRecentActivities(limit);
      res.json(activities);
    } catch (error) {
  ...
  app.post("/api/activities", async (req, res) => {
    try {
      const activity = await dataStorage.createActivity(req.body);
      res.json(activity);
    } catch (error) {
  ...
  app.post("/api/new-lead", async (req, res) => {
    try {
      ...
      const business = await dataStorage.createBusiness(businessData);
        ...
        if (fullDateTime) {
          try {
            ...
            if (parsedDate.isValid()) {
              ...
              await dataStorage.updateBusiness(business.id!, {
                stage: 'scheduled',
                scheduledTime: parsedDate.toISOString()
              });
              
              // Log activity
              await dataStorage.createActivity({
      ...
      setTimeout(async () => {
        try {
          ...
          const enrichedBusiness = await dataStorage.getBusiness(business.id);
  ...
  app.post("/api/appointment-webhook", async (req, res) => {
    try {
      ...
      if (action === "cancelled") {
        try {
          ...
          if (existingAppointment) {
            ...
            await dataStorage.updateBusiness(existingAppointment.businessId, {
              stage: 'scraped',
      ...
      if (business) {
        ...
        await dataStorage.updateBusiness(existingBusinessId, {
          stage: 'scheduled',
        ...
        business = await dataStorage.createBusiness(businessData);
        existingBusinessId = business.id!;
      }
  ...
  app.post("/api/bot/score/:id", async (req, res) => {
    try {
      ...
      const business = await dataStorage.updateBusiness(id, {
        score,
        tags,
        priority,
        notes: `Score: ${score}\nTags: ${tags.join(", ")}\nPriority: ${priority}\n\n${(await dataStorage.getBusiness(id))?.notes || ""}`,
      });
  ...
  app.post("/api/bot/launch-outreach", async (req, res) => {
    try {
      ...
      const campaign = await dataStorage.createCampaign({
      ...
      await dataStorage.updateCampaign(campaign.id, {
  ...
  app.get("/api/businesses/:id/activities", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const activities = await dataStorage.getBusinessActivities(id);
      res.json(activities);
    } catch (error) {
  ...
  app.post("/api/schedule", async (req, res) => {
    try {
      ...
      const allBusinesses = await dataStorage.getBusinesses();
      ...
      const existingBusiness = await dataStorage.getBusinessById(business_id);
      ...
      const business = await dataStorage.updateBusiness(business_id, {
        stage: 'scheduled',
        scheduledTime: datetime,
        appointmentStatus: 'confirmed'
      });
  ...
  app.get("/api/availability", async (req, res) => {
    try {
      const availability = await dataStorage.getAvailabilityConfig();
  ...
  app.post("/api/availability", async (req, res) => {
    try {
      ...
      await dataStorage.updateAvailabilityConfig(configs);
  ...
  app.get("/api/scheduling/link/:businessId", async (req, res) => {
    try {
      ...
      const business = await dataStorage.getBusinessById(businessId);
  ...
  app.get("/api/blocked-dates", async (req, res) => {
    try {
      const blockedDates = await dataStorage.getBlockedDates();
  ...
  app.post("/api/blocked-dates", async (req, res) => {
    try {
      ...
      const blockedDate = await dataStorage.createBlockedDate({
        date,
        startTime,
        endTime,
        reason
  ...
  app.delete("/api/blocked-dates/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      await dataStorage.deleteBlockedDate(id);
      res.status(204).send();
    } catch (error) {
  ...
  app.get("/api/scheduling/slots", async (req, res) => {
    try {
      ...
      const leadScore = businessId ? 
        (await dataStorage.getBusinessById(parseInt(businessId as string)))?.score || 0 : 
        0;
  ...
  app.post("/api/messages/send", async (req, res) => {
    try {
      ...
      const business = await dataStorage.getBusinessById(leadId);
      if (!business) {
        return res.status(404).json({ error: "Business not found" });
      }
      ...
      await dataStorage.updateBusiness(leadId, {
  ...
  app.patch("/api/scheduling/appointments/:id/status", async (req, res) => {
    try {
      ...
      // Update status
      const updatedBusiness = await dataStorage.updateBusiness(businessId, {
  ...
  app.post("/api/appointments", async (req, res) => {
    try {
      ...
      await dataStorage.updateBusiness(businessId, {
        stage: 'scheduled',
      });
  ...
  app.patch("/api/appointments/:id", async (req, res) => {
    try {
      ...
      if (status === 'no-show') {
        const business = await dataStorage.getBusinessById(appointment.businessId);
        if (business) {
          // Send reschedule SMS
  ...
  app.delete("/api/appointments/:id", async (req, res) => {
    try {
      ...
      if (otherAppointments.length === 0) {
        await dataStorage.updateBusiness(appointment.businessId, {
          stage: 'contacted',
        });
      }
  ...
  app.get("/api/businesses/:id/messages", async (req, res) => {
    try {
      ...
      // Get activities that are message-related
      const messages = await db.select()
        .from(schema.activities)
        .where(
          and(
            eq(schema.activities.businessId, businessId),
            or(
              eq(schema.activities.type, 'sms_sent'),
              eq(schema.activities.type, 'sms_received')
            )
          )
        )
      ...
      // Transform activities into message format with sender info
      const formattedMessages = messages.map(activity => {
        // Extract sender info from the message format [SenderName] Message
        let message = activity.description;
        let senderName = activity.type === 'sms_sent' ? 'Admin' : 'Client';
        
        // Check if message has sender info in [SenderName] format
        const senderMatch = activity.description.match(/^\[(.*?)\]\s(.*)/);
        ...
        return {
          ...
          direction: activity.type === 'sms_sent' ? 'outbound' : 'inbound',
          status: 'delivered',
          senderName: senderName,
          senderId: `user-${activity.id}` // Generate a consistent ID for the sender
        };
      });
  ...
  // Debug bridge for Squarespace widget messages
  app.post("/api/bridge/messages", async (req: Request, res: Response) => {
    try {
      const { projectToken, content, senderName, senderType } = req.body;
      
      console.log("🔄 [BRIDGE] Received message from Squarespace widget:");
      console.log(`🔄 [BRIDGE] projectToken: ${projectToken}`);
      console.log(`🔄 [BRIDGE] content: ${content}`);
      console.log(`🔄 [BRIDGE] senderName: ${senderName}`);
      console.log(`🔄 [BRIDGE] senderType: ${senderType}`);
      
      // For testing, always map to business ID 1
      const businessId = 1;
      
      // Store the message in the database
      const messageWithSender = `[${senderName}] ${content}`;
      await dataStorage.createActivity({
        type: senderType === 'admin' ? 'sms_sent' : 'sms_received',
        description: messageWithSender,
      ...
      io.to('admin').emit('new:message', {
        ...
        message: content,
        timestamp: new Date().toISOString(),
        direction: senderType === 'admin' ? 'outbound' : 'inbound',
        senderName,
      ...
      res.json({ 
        ...
        bridged: true,
        businessId,
        message: "Message bridged to WebsiteWizard"
      });
    } catch (error) {
      console.error("Error in bridge endpoint:", error);
      res.status(500).json({ error: "Failed to bridge message" });
    }
  });

  // File upload endpoint for messaging
  ...
  io.use((socket: Socket, next: (err?: Error) => void) => {
    ...
    // If token is provided, verify it's an admin token
    ...
    // If businessId is provided, it's a client connection
    if (businessId) {
      socket.data.businessId = businessId;
      console.log(`Client socket connected for business: ${businessId}`);
      return next();
    }
    
    // Neither admin token nor businessId provided
  ...
  io.on('connection', (socket: Socket) => {
    ...
    // Join appropriate rooms based on auth
    if (socket.data.isAdmin) {
      socket.join('admin');
      socket.emit('authenticated', { role: 'admin' });
    } else if (socket.data.businessId) {
      const roomId = `business:${socket.data.businessId}`;
      socket.join(roomId);
      socket.join('clients');
      socket.emit('authenticated', { role: 'client', businessId: socket.data.businessId });
    }
    
    // Handle client message
    socket.on('client:message', async (data: {businessId: string|number, message: string, senderName?: string, senderId?: string}) => {
      try {
        const { businessId, message, senderName, senderId } = data;
        ...
        // Ensure businessId is a number for database operations
        const businessIdNum = typeof businessId === 'string' ? parseInt(businessId, 10) : businessId;
        
        // Get the business info
        const business = await dataStorage.getBusiness(businessIdNum);
        if (!business) {
          socket.emit('error', { message: 'Business not found' });
          return;
        }
        
        // Use provided sender name or fall back to business name
        const finalSenderName = senderName || business.name;
        
                 // Store the message in the database with sender info
         // Format the description to include sender info
         const messageWithSender = `[${finalSenderName}] ${message}`;
         await dataStorage.createActivity({
           type: 'sms_received',
           description: messageWithSender,
    ...
    socket.on('admin:message', async (data: {businessId: string|number, message: string, senderName?: string, senderId?: string}) => {
      try {
        const { businessId, message, senderName = 'Admin', senderId = 'admin-user' } = data;
        ...
        if (!business) {
         ...
         const activity = await dataStorage.createActivity({
           type: 'sms_sent',
           description: messageWithSender,
           businessId: businessIdNum
         });
        
        // Send to specific business room
        ...
        // Also echo back to all admins
        io.to('admin').emit('message:sent', {
          businessId,
          businessName: business.name,
          message,
          timestamp: new Date().toISOString(),
